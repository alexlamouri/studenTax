...
class CSVModel():
    ...
    def save_record(self, data):
        fields = {'Employment Income': data[0], 'Employment Insurance':data[1], 'Total Income':data[2], 'Net Income':data[3],
        'Other Payments deduction': 0, 'Taxable Income':data[4],'Basic Personal Amount':0,
        'Canada Employment amount':0, 'Tuition':0, 'Total non-refundable':data[5],'total payable':0, 
        'total income tax deducted':0, 'tax paid instalments':data[6], 'total refundable credits':0, 'refund':data[7]}

        # newfile = not os.path.exists(self.filename)
        with open(self.filename, 'a') as fh: 
            csvwriter = csv.DictWriter(fh, fieldnames= self.fields.key())
        if newfile:
